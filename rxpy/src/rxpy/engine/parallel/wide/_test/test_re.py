...
import rxpy.engine.parallel.wide.re as re
from rxpy.engine.backtrack.re import Scanner
...
#        self.assertNotEqual(scanner.scanner.scanner("").pattern, None)
    ...
    def test_bug_725149(self):
        ...
        self.assertEqual(re.match('(a)(?:(?=(b)*)c)*', 'abb').groups(),
                         ('a', None))
        self.assertEqual(re.match('(?_e)(a)((?!(b)*))*', 'abb').groups(),
                         ('a', None, None))
...
def run_re_tests():
    from rxpy.engine.backtrack._test.re_tests import tests, SUCCEED, FAIL, SYNTAX_ERROR
#    if verbose:
